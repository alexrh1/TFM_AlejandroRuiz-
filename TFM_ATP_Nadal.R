...
#Para evaluar el pseudo-R2 en regr. logística 
...
impVariablesLog<-function(modelo,nombreVar,dd=data_train){
  ...
  barplot(aux3$V5,names.arg = aux3$V2,las=2,horiz=T,col="dodgerblue",main="Importancia de las variables (Pseudo-R2)")
}
...
modeloForwAIC<-step(full, scope=list(lower=null, upper=full), direction="forward")
modeloStepAIC<-step(null, scope=list(lower=null, upper=full), direction="both")

roc(data_test$win_or_lose, predict(modeloForwAIC,data_test,type = "response"), direction="<")
roc(data_test$win_or_lose, predict(modeloStepAIC,data_test,type = "response"), direction="<")
compararLog(modeloForwAIC,data_test,"win_or_lose",'1')
compararLog(modeloStepAIC,data_test,"win_or_lose",'1')
pseudoR2(modeloForwAIC,data_test,"win_or_lose") #0.31
pseudoR2(modeloForwAIC,data_test,"win_or_lose") #0.67
...
## Regresion LASSO
y <- as.double(as.matrix(data_train[, 1])) # 1 es la columna de la Variable Objetivo
...
#modificamos los datos en  test para obtener su AUC y usamos otra funcion para el resto de estadísticos
...
## Validacion cruzada repetida
total<-c()
modelos<-list(modeloInicial,modeloManual,modeloForwAIC,modeloStepAIC,modeloStepBIC,modeloStepIntAIC,modeloForwIntAIC, modeloStepIntBIC,modeloForwIntBIC)
...
boxplot(accuracy~modelo,data=total,main="Tasa de acierto", col="dodgerblue")
axis(3, at=1:length(modelos), labels=sapply(modelos,function(x) length(coef(x))), cex.axis=1)
boxplot(roc~modelo,data=total,main="Área bajo la curva ROC", col="dodgerblue")
axis(3, at=1:length(modelos), labels=sapply(modelos,function(x) length(coef(x))), cex.axis=1)
aggregate(roc~modelo, data = total, mean) 
aggregate(roc~modelo, data = total, sd)

#vemos la fórmula y las variables máás importantes
formula(modeloStepBIC) 
par(mar=c(5.1, 7, 4.1, 2.1))#ajusto los márgenes
impVariablesLog(modeloStepBIC,"win_or_lose")
compararLog(modeloStepBIC,data_test,"win_or_lose",'1')
modeloStepBIC
#calculo los odds ratio 
...
plot(test_roc,print.thres="best") #punto de corte que maximiza el indice de youden
plot(test_roc,print.thres=c(0.745,0.5)) #comparo con 0.5 el que he observado antes
